// src/pages/instructor/content/ContentFavoritePage.tsx
// 교수자 > 찜한 콘텐츠 목록 페이지 (Mock 데이터 + 카드 그리드)
import { useState, useMemo } from 'react';
import { useTranslation } from '@/i18n';
import { Search, Heart, Play, FileText, HelpCircle, ClipboardList, Eye, ThumbsUp, Clock } from 'lucide-react';
...
const mockFavorites: Content[] = [
  { id: 1, title: 'Python 기초 문법 강의', description: '변수, 자료형, 조건문, 반복문 등 Python 핵심 문법을 학습합니다.', type: 'video', thumbnail: '🐍', tags: ['Python', '프로그래밍', '입문'], duration: '45분', views: 1250, likes: 89, createdAt: '2026-01-15' },
  { id: 3, title: 'React 컴포넌트 실습', description: 'useState, useEffect 등 React Hook을 활용한 컴포넌트 개발 실습입니다.', type: 'video', thumbnail: '⚛️', tags: ['React', 'Frontend', 'Hook'], duration: '60분', views: 2100, likes: 156, createdAt: '2026-01-22' },
  { id: 5, title: 'REST API 설계 실습', description: 'RESTful API 설계 원칙과 Spring Boot 구현 과제입니다.', type: 'assignment', thumbnail: '🌐', tags: ['API', 'Spring Boot', 'REST'], duration: '과제 2시간', views: 430, likes: 28, createdAt: '2026-01-28' },
  { id: 8, title: 'UX/UI 디자인 원칙', description: '사용자 경험 중심의 UI 설계 원칙을 정리한 문서입니다.', type: 'document', thumbnail: '🎨', tags: ['UX', 'UI', '디자인'], duration: '읽기 15분', views: 560, likes: 41, createdAt: '2026-02-05' },
];
...
export default function ContentFavoritePage() {
  const { t } = useTranslation();
  const [searchTerm, setSearchTerm] = useState('');
  const [removed, setRemoved] = useState<Set<number>>(new Set());

  const filtered = useMemo(() => {
    return mockFavorites.filter((c) => {
      if (removed.has(c.id)) return false;
      return searchTerm === '' || c.title.includes(searchTerm) || c.tags.some(tag => tag.includes(searchTerm));
    });
  }, [searchTerm, removed]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.length === 0 ? (
          <div className="col-span-full py-20 text-center">
            <Heart className="w-10 h-10 text-gray-300 dark:text-slate-600 mx-auto mb-2" />
              ...
              <div key={item.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden hover:shadow-md transition-shadow">
                ...
                <div className="relative h-36 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-slate-700 dark:to-slate-800 flex items-center justify-center">
                  ...
                  <button
                    ...
                    title={t('instructor.removeFavorite')}
                  >
