...
info.put("onoff_type_conv", m.getItem(info.s("onoff_type"), course.onoffPackageTypes));
...
while(lessons.next()) {
	...
	lessons.put("onoff_type_conv", m.getItem(lessons.s("onoff_type"), lesson.onoffTypes));
...
while(exams.next()) {
	exams.put("item_name", m.getItem(exams.s("item_type"), courseModule.examTypes));
}
...
while(homeworks.next()) {
	homeworks.put("item_name", m.getItem(homeworks.s("item_type"), courseModule.homeworkTypes));
}
...
while(forums.next()) {
	forums.put("item_name", m.getItem(forums.s("item_type"), courseModule.forumTypes));
}
...
while(surveys.next()) {
	surveys.put("item_name", m.getItem(surveys.s("item_type"), courseModule.surveyTypes));
}
...
while(modules.next()) {
	...
	modules.put("module_conv", m.getItem(modules.s("module"), courseModule.evaluations));
}
...
while(courses.next()) {
	...
	courses.put("course_type_conv", m.getItem(courses.s("course_type"), course.types));
	courses.put("onoff_type_conv", m.getItem(courses.s("onoff_type"), course.onoffPackageTypes));
...
//출력
p.setLayout(ch);
//if("P".equals(info.s("onoff_type"))) { p.setBody("mobile.package_view"); }
//else { p.setBody("mobile.course_view"); }
p.setBody("mobile.course_view");
p.setVar("query", m.qs());
...
p.setVar("btn_style", !"".equals(info.s("sl_start_url")) ? "r_view" : "wp100");

p.setVar("section_colspan", 2);
