...
KollusDao kollus = new KollusDao(siteinfo.s("access_token"), siteinfo.s("security_key"));
...
while(rs.next()) {
	...
	if(rs.i("kind") == 1) {
		...
		if(info.next()) {
			ret.put("expiration_date", m.getUnixTime(info.s("end_date")));
			ret.put("result", 1);
		} else {
			ret.put("result", 0);
...
String res = ret.serialize();
out.print(kollus.getKollusEncrypt(res));
