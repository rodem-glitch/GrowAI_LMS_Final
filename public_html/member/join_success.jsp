...
uinfo.put("sms_yn_conv", m.getValue(uinfo.s("sms_yn"), user.receiveYnMsg));
uinfo.put("email_yn_conv", m.getValue(uinfo.s("email_yn"), user.receiveYnMsg));
uinfo.put("reg_date_conv", m.time(_message.get("format.datetime.local"), uinfo.s("reg_date")));

//출력
p.setLayout(!isGoMobile ? ch : "mobile");
p.setBody((!isGoMobile ? "member" : "mobile") + ".join_success");
