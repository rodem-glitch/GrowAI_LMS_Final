...
//객체
Auth2LogDao auth2Log = new Auth2LogDao(siteId, "user");
...
String auth2Type = m.rs("auth2_type", siteinfo.s("user_auth2_type"));
//String auth2Type = auth2Info.s("authType");

String msg = "";

//2022.01.20. 추후 사용자단 OTP 추가 예정
if("O".equals(auth2Type)) { m.jsError("해당 인증방식이 없습니다."); return; }
...
//출력
p.setLayout("blank");
p.setBody("member.auth2");
p.setVar("form_script", f.getScript());
p.setVar("query", m.qs());
p.setVar("returl", returl);
p.setVar("domain", request.getServerName());
p.setVar("auth2_type", auth2Type);
p.setVar("auth2_multitype_block", "Y".equals(SiteConfig.s("user_auth2_multitype_yn")));
