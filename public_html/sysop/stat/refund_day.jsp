...
//접근권한
if(!Menu.accessible(64, userId, userKind)) { m.jsError("접근 권한이 없습니다."); return; }
...
DataSet stat = refund.query(
	"SELECT SUBSTRING(a.refund_date, 1, 8) refund_date, a.refund_method, SUM(a.refund_price) price, COUNT(*) cnt "
	+ " FROM " + refund.table + " a "
...
if("excel".equals(m.rs("mode"))) {
	...
	ExcelWriter ex = new ExcelWriter(response, "일별환불통계(" + m.time("yyyy-MM-dd") + ").xls");
	ex.setData(list, (String [])v.toArray(new String[v.size()]), "일별환불통계 (" + m.time("yyyy-MM-dd") + ")");
	ex.write();
	return;
}

//출력
p.setBody("stat.refund_day");
p.setVar("query", m.qs());
