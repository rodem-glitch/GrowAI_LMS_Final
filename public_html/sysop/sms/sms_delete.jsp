...
if(!"".equals(m.rs("idx"))) { //복수삭제
	...
	DataSet list = sms.find("id IN (" + idx + ") AND status = 1 AND site_id = " + siteId + "");
	if(list.size() == 0) { m.jsError("해당 정보가 없습니다."); return; }

	if(-1 == sms.execute("UPDATE " + sms.table + " SET status = -1 WHERE id IN ( " + idx + " )")) {
	...
	if(-1 == sms.execute("UPDATE " + sms.table + " SET status = -1 WHERE id = " + id + "")) {
		m.jsError("삭제하는 중 오류가 발생했습니다.");
		return;
	}
