...
//기본키
int fid = m.ri("fid");
if(courseId == 0 || fid == 0) { m.jsErrClose("기본키는 반드시 있어야 합니다."); return; }
...
DataSet info = courseModule.query(
	...
	+ " AND a.course_id = " + courseId + " AND a.module_id = " + fid + ""
);
...
DataSet stat = courseModule.query(
	...
	+ " LEFT JOIN " + forumUser.table + " e "
		+ " ON e.forum_id = " + fid + " AND e.course_user_id = a.id AND e.status = 1 "
...
DataSet items = courseModule.query(
	...
	+ " WHERE eu.forum_id = " + fid + " AND eu.course_id = " + courseId + " "
...
//출력
p.setLayout("pop");
p.setBody("management.stat_forum");
p.setVar("p_title", "토론 성적분포");
