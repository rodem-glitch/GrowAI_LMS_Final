...
DataSet info = courseModule.query(
	...
	+ " INNER JOIN " + forum.table + " f ON a.module_id = f.id "
...
info.put("forum_file_url", m.getUploadUrl(info.s("forum_file")));
info.put("forum_file_ek", m.encrypt(info.s("forum_file") + m.time("yyyyMMdd")));

//현황
DataSet stat = courseModule.query(
	"SELECT COUNT(*) u_cnt "
	+ ", SUM( CASE WHEN f.submit_yn = 'Y' THEN 1 ELSE 0 END ) s_cnt "
	+ ", SUM( CASE WHEN f.submit_yn = 'Y' AND f.confirm_yn = 'Y' THEN 1 ELSE 0 END ) c_cnt "
	+ ", SUM( CASE WHEN f.submit_yn = 'Y' AND f.confirm_yn = 'Y' THEN f.marking_score ELSE 0 END ) t_score "
	+ " FROM " + courseUser.table + " a "
	+ " INNER JOIN " + user.table + " u ON u.id = a.user_id " + (deptManagerBlock ? " AND u.dept_id IN (" + userDept.getSubIdx(siteId, userDeptId) + ") " : "")
	+ " LEFT JOIN " + forumUser.table + " f "
		+ " ON f.forum_id = " + id + " AND f.course_user_id = a.id AND f.status = 1 "
...
//출력
p.setBody("management.forum_view");
