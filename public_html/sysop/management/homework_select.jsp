...
if("add".equals(m.rs("mode"))) {
	...
	//제한
	if(0 < courseModule.findCount("course_id = " + courseId + " AND module = 'homework' AND module_id = " + id + " AND status != -1")) {
		m.jsAlert("해당 과제는 이미 배정되어 있습니다.");
	...
	int totalAssignScore = courseModule.getOneInt(
		...
		+ " WHERE course_id = " + courseId + " AND module = 'homework' AND status = 1 "
	);
	int assignScore = cinfo.i("assign_homework") - totalAssignScore;
	...
	courseModule.item("module_nm", info.s("homework_nm"));
...
lm.addWhere("NOT EXISTS ( "
	...
	+ " WHERE course_id = " + courseId + " AND module = 'homework' AND module_id = a.id "
+ " )");
...
//출력
p.setLayout("pop");
p.setBody("management.homework_select");
p.setVar("p_title", "과제 추가");
p.setVar("query", m.qs());
