...
//폼입력
int mid = m.ri("mid");
String module = m.rs("md", "board");
String mode = m.rs("mode");
int id = m.ri("id");

//기본키
if(mid == 0) { m.jsErrClose("기본키는 반드시 지정해야 합니다."); return; }
...
//최대값
int maxSort = category.findCount("module_id = " + mid + " AND module = '" + module + "' AND status = 1");
...
//	category.item("sort", f.get("sort"));
	...
	//삭제
		...
		//순서정렬
		category.autoSort(module, mid, siteinfo.i("id"));
	}

	m.jsReplace("category.jsp?" + m.qs(), "parent");
	return;
}
...
//출력
p.setLayout("pop");
p.setBody("board.category");
p.setVar("p_title", "카테고리 관리");
p.setVar("form_script",  f.getScript());
p.setVar("query", m.qs());
p.setVar("list_query", m.qs("id"));

p.setLoop("list", category.find("site_id = " + siteId + " AND module_id = " + mid + " AND module = '" + module + "' AND status = 1", "*", "sort ASC"));
