...
DataSet list = category.query(
	...
	+ " LEFT JOIN ( "
		+ " SELECT parent_id, COUNT(*) cnt FROM " + category.table + " WHERE status = 1 AND module = 'book' GROUP BY parent_id "
	+ " ) b ON a.id = b.parent_id"
	+ " WHERE a.status = 1 AND a.module = 'book' AND a.site_id = " + siteId + " "
...
//출력
p.setLayout("pop");
p.setBody("book.category_select");
p.setVar("p_title", "카테고리 선택");
p.setLoop("list", list);

p.setVar(mode + "_block", true);
p.setVar("root_cnt", category.findCount("site_id = " + siteId + " AND status = 1 AND module = 'book' AND depth = 1") + 1);
p.display();
