...
//정보
DataSet info = category.find("id = " + id + " AND status = 1 AND module = 'book' AND site_id = " + siteId + "");
...
//정보-상위
DataSet pinfo = category.find("id = " + pid + " AND status = 1 AND module = 'book' AND site_id = " + siteId +"");
...
DataSet list = book.find("category_id IN (" + category.getSubIdx(siteId, id) + ") AND status != -1 ORDER BY sort, id DESC");
while(list.next()) {
