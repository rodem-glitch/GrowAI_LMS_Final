...
//객체
CommentDao comment = new CommentDao(siteId);
Json j = new Json(out);
...
if(m.isPost()) {
    if("comment_add".equals(mode)) {
        ...
        if(f.validate()) {
            ...
            if(!comment.update("module = '" + module + "' AND module_id = " + moduleId + " AND id = " + info.i("id") + " AND status = 1")) { j.print(-1, "수정하는 중 오류가 발생하였습니다."); return; }
        ...
        //삭제
        comment.item("status", -1);
        if(!comment.update("module = '" + module + "' AND module_id = " + moduleId + " AND id = " + info.i("id") + " AND status = 1")) { j.print(-1, "삭제하는 중 오류가 발생하였습니다."); return; }
