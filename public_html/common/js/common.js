...
function CloseLayer(callback) {
	...
	*/
	var ifrm = gi("wrap-layer" + TopLayer).getElementsByTagName("iframe")[0];
	//var ifrm = document.getElementById("wrap-layer" + TopLayer).getElementsByTagName("iframe")[0];
...
function initTinyMCE(obj) {
	...
	let validElem = "";
	if(typeof selector == 'undefined' || selector === "") selector = "#content, textarea.tinymce";
	...
	tinymce.init({
		...
		plugins: "advlist autolink lists code anchor table link fullscreen"
			+ (modal ? " image media" : "")
			+ " paste",
		font_formats: "맑은고딕=Malgun Gothic;나눔고딕=Nanum Gothic;Arial=arial,helvetica,sans-serif; Arial Black=arial black,avant garde; Book Antiqua=book antiqua,palatino; Comic Sans MS=comic sans ms,sans-serif; Courier New=courier new,courier; Georgia=georgia,palatino; Helvetica=helvetica; Impact=impact,chicago; Symbol=symbol; Tahoma=tahoma,arial,helvetica,sans-serif; Terminal=terminal,monaco; Times New Roman=times new roman,times; Trebuchet MS=trebuchet ms,geneva; Verdana=verdana,geneva;",
	...
	$(document).ready(function() {
		$("textarea").each(function() {
			...
			if(-1 < $.inArray(theme, theme_ch)) {
				...
				if(theme == "advanced" || theme == "full" || theme == "default") initSummernote($(this), "summernote");
				else if(theme == "simple" || theme == "user") initSummernote($(this), "summernote_nomodal");
				else initSummernote($(this), "summernote");
...
function initSummernote(el, theme) {
	if(typeof $.summernote != "undefined") {
		var toolbars = null;
		var obj = {
			modal: theme !== "summernote_nomodal",
		};
...
function call(url, id, callback) {
	...
	if(window.ActiveXObject) {
		try {
			...
			} catch(e) { console.log('call ActiveXObject initialize error : ' + e); }
		}
	} else {
...
//					el.innerHTML = client.responseText;
				...
				for(var i=0; i<scripts.length; i++) {
					scripts[i].innerHTML = (scripts[i].innerHTML.replace("<!--", "").replace("-->", ""));
				}
			}
		}
		var f;
    ...
    var ready_event_listener = function (fn) {
        ...
        if (document.addEventListener) {
           ...
           // document.attachEvent("onreadystatechange", idempotent_fn);
            ...
            var toplevel = false;
            try { toplevel = window.frameElement == null; } catch (e) { console.log('domframe attachEvent error : ' + e); }
