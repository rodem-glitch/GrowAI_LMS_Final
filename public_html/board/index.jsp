...
lm.addWhere("a.board_id = ?", new Object[] {bid});
...
if(!"".equals(f.get("s_field")) && m.inArray(f.get("s_field"),"a.subject,a.content,a.writer")) {
	lm.addSearch(f.get("s_field"), f.get("s_keyword"), "LIKE");
} else if("".equals(f.get("s_field")) && !"".equals(f.get("s_keyword"))) {
	lm.addSearch("a.subject,a.content,a.writer", f.get("s_keyword"), "LIKE");
}
if(!"".equals(f.get("ord")) && m.inArray(m.replace(f.get("ord"), " ", ""), "a.subject asc,a.subject desc,a.reg_date asc,a.reg_date desc,a.sort asc,a.sort desc")) {
	lm.setOrderBy(f.get("ord"));
} else {
	lm.setOrderBy(("faq".equals(btype) ? "a.sort ASC, " : "") + "a.thread asc, a.depth asc");
}
...
while(list.next()) {
	...
	list.put("subject_conv", list.s("subject"));
	...
	if("board".equals(btype)) {	//리스트형
		...
		while(files.next()) {
			if("Y".equals(binfo.s("image_yn"))) p.setVar("image_block", true);
...
if(binfo.b("notice_yn")) {
	notices = post.query(
		...
		+ " LEFT JOIN " + file.table + " f ON f.module = 'post' AND f.module_id = a.id AND f.main_yn = 'Y' "
		+ " LEFT JOIN " + user.table + " u ON a.user_id = u.id "
		+ " WHERE a.display_yn = 'Y' AND a.status = 1 AND a.board_id = " + bid + " "
		+ " AND a.notice_yn = 'Y' "
	...
	while(notices.next()) {
		...
		notices.put("subject_conv", m.cutString(notices.s("subject"), 60));
